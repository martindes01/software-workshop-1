...
public class RockPaperScissors {
    ...
    /**
     * This method checks that the parameter 'turn' represents a valid turn. A valid
     * turn is in the range 1 to 3 inclusive. A value of 1 means ROCK; a value of 2
     * means PAPER and a value of 3 means SCISSORS. Each turn value is, therefore,
     * represented as an integer. Any value of 'turn' outside of this range is
     * invalid.
     *
    ...
    /**
     * This method effectively converts the keyboard input into a valid turn. The
     * keyboard input is in the form of a String and will represent the key pressed
     * by the player. The player presses 'r' for rock, 'p' for paper and 's' for
     * scissors. This method converts that input into a valid turn, represented as
     * an integer in the range 1-3. If the parameter 'input' does not contain 'r' or
     * 'p' or 's' this method should return the value -1.
     *
     * @param input The turn selected by the player. Options described above.
     * @return The turn as an integer. 1 means rock, 2 means paper, 3 means
     *         scissors. A return value of -1 means that the parameter was invalid.
     */
    public int getPlayerTurn(String input) {
        ...
        // input is null or does not contain 'r', 'p' or 's'
        return -1;
    }

    /**
     * This method decides which of the two players - the player or the computer -
     * has won the round. The winner is decided as follows: - rock beats scissors -
     * paper beats rock - scissors beats paper - if the the players choose the same
     * object the round is a draw.
     *
     * @param playerChoice   The player's turn.
     * @param computerChoice The computer's turn.
     * @return The winner, according to the rules above. If the player wins the
     *         method should return 'PLAYER', if the computer wins the method should
     *         return 'COMPUTER', and if it's a draw the method should return
     *         'DRAW'.
     */
    public String getWinner(int playerChoice, int computerChoice) {
        ...
        switch (playerChoice) {
            ...
            case IPAPER:
                switch (computerChoice) {
                    case IROCK:
                        return PLAYER; // paper beats rock
                    case ISCISSORS:
                        return COMPUTER; // scissors beat paper
                }
            case ISCISSORS:
                switch (computerChoice) {
                    case IPAPER:
                        return PLAYER; // scissors beat paper
                    case IROCK:
    ...
    /**
     * This method effectively converts a turn represented as an integer to a
     * String. If the parameter has the value 1 the method should return 'ROCK'. If
     * the parameter has the value 2 the method should return 'PAPER'. If the
     * parameter has the value 3 the method should return 'SCISSORS'.
     *
    ...
    private void run() {
        ...
        while (!splayerTurn.equals("x")) {
            ...
            if (!isValidTurn(playerTurn)) {
                ...
                System.out.println("The player chose " + getTurnAsString(playerTurn) + " and the computer chose "
                        + getTurnAsString(computerTurn));
    ...
    /**
     * This method records a win for either the player or the computer based on the
     * parameter 'result'. If the value of the parameter is 'PLAYER' the method
     * increments the number of player wins. If the value of the parameter is
     * 'COMPUTER' the method increments the number of computer wins.
     *
